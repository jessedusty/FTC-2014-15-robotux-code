...
task inputBall() {
	...
	int currentRollerMotorSpeed = motor[rollerMotor];
	motor[rollerMotor] = 0;
	...
	wait1Msec(200);
	motor[rollerMotor] = currentRollerMotorSpeed;
...
void accessoryControl1() {
	if (abs(joystick.joy1_y2) > 10 && !busyInputingBall) {
...
void accessoryControl2() {
	...
	if (joystick.joy2_TopHat == 2 && !busyInputingBall) {
		servo[flipperServo] = 90;
		} else if (joystick.joy2_TopHat == 6 && !busyInputingBall) {
		servo[flipperServo] = 180;
		} else {
		//servo[servo3] = flipperTarget;
	}
...
/*	if (joy2Btn(4)) {
	...
	if (joy2Btn(4)) {
		resetMotorEncoders();
		nMotorEncoderTarget[liftMotor1] = 50;
		nMotorEncoderTarget[liftMotor2] = 50;
		motor[liftMotor1] = -100;
		motor[liftMotor2] = -100;
		ClearTimer(T1);

		while(nMotorRunState[liftMotor1] != runStateIdle&&nMotorRunState[liftMotor2] != runStateIdle&&time1[T1] < 5000)
		{
			  // do not continue
		}
		motor[liftMotor1] = 0;
...
void encoderPositionDebugging() {
	...
	heightEncoder = nMotorEncoder[liftMotor2];
}
...
void protoboard() {
	if (!override){
		...
		//lightSensor = 40;
	/*	if (lightSensor < 65 && lightSensor > 30 && !busyInputingBall) {
			StartTask(inputBall);
		}*/
	}
	else
