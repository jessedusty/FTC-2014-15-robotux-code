...
void joyval_joystick1st() { // standard mode on left joystick
	...
	rightstickval = pow(((float)joystick.joy1_x2 / (float)64 * (float)5), 2) ;
...
void accessoryControl() {
	if (abs(joystick.joy1_y1) > 5 && !busyInputingBall) {
		motor[flapTrackMotor] = joystick.joy1_y1;
	...
	if (joy1Btn(11)) {
		...
		servo[servo4] = drawBridgeRestingState;
	}
	//stops roller and belt
	if (joy1Btn(7)){
	busyInputingBall=false;
	motor[rollerMotor] = 0;
	rollerRolling = 0;
